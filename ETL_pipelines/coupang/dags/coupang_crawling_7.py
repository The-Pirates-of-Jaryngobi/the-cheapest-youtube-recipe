...
def import_ingredient_name_table():
    ...
    ingredient_name_list = ingredient_name_list[(len(ingredient_name_list)//10)*6:(len(ingredient_name_list)//10)*7]
...
def write_to_csv(ingredient_result):
    ...
    file_name = f'ingredient_result_7_{current_time.strftime("%Y-%m-%d_%H-%M-%S")}.csv'
...
def load_to_rds(ingredient_result):
    ...
    DROP TABLE IF EXISTS product_7;
    CREATE TABLE product_7 (
