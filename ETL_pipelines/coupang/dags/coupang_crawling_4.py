...
def import_ingredient_name_table():
    ...
    ingredient_name_list = ingredient_name_list[(len(ingredient_name_list)//10)*3:(len(ingredient_name_list)//10)*4]
...
def write_to_csv(ingredient_result):
    ...
    file_name = f'ingredient_result_4_{current_time.strftime("%Y-%m-%d_%H-%M-%S")}.csv'
