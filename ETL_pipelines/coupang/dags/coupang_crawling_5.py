...
def import_ingredient_name_table():
    ...
    ingredient_name_list = ingredient_name_list[(len(ingredient_name_list)//10)*4:(len(ingredient_name_list)//10)*5]
...
def write_to_csv(ingredient_result):
    ...
    file_name = f'ingredient_result_5_{current_time.strftime("%Y-%m-%d_%H-%M-%S")}.csv'
